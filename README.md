...
> `raw.hellogithub.com` 背后是 CDN 智能调度，会根据你的网络出口 IP 自动返回所在地区测速最优的 GitHub IP。北京联通用户和广州移动用户拉到的列表是不同的，无需额外配置。

> 如果你开了全局代理，出口 IP 会变成代理所在地，拉到的列表就不再适合直连使用。建议关掉代理后再运行本工具。
